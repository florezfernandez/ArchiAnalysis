...
public class RTO extends AbstractArchiAnalysisFunction{

	private List<RTOResult> aResult;
	private List<WService> wElements;

	@Override
	public String getName(){return "Estimate overall architecture RTO";}
	@Override
	...
	public Object executeFunction() throws Exception {
		aResult = new  ArrayList<RTOResult>();
		for (WService wService : wElements) {
			RTOResult a = new RTOResult(wService.getElement());
			a.categorization = wService.getCategorization();
			a.rto = Utils.getRTO(wService);
			aResult.add(a);			
		}
		return null;
	}
	...
	public Object showResults () throws Exception{
		String[] headers = new String[]{"Element Type","Element","Categorization","Recovery time objective (max. outage)"};
