...
import co.edu.uniandes.archimate.analysis.functional.hr.decompositionFunctional.RaciMatrixResult;
...
public class DecompositionFunctional extends AbstractArchiAnalysisFunction implements IChainableArchiFunction{
	private List<DecompositionFunctionalResult> listResults;
	private HashMap<String,WElement> elements1;
	private HashMap<String,WElement> elements2;
	private List<WElement> orderedElements2;
	private ResultSerializable orgUnitVsActorMatrix;
	private ResultSerializable actorVsRoleMatrix;
	private ResultSerializable raciMatrix;
	...
	public Object validateModel(ValidationResponse validationResponse) throws Exception{
		DecompositionFunctionalxSWT mSWT=new DecompositionFunctionalxSWT(Display.getCurrent());
		mSWT.displayWidget();
		String resultOrgUnitVsActorMatrix= mSWT.getFileorgUnitVsActorMatrix();
		String resultActorVsRoleMatrix= mSWT.getFileactorVsRoleMatrix();
		String resultRaciMatrix= mSWT.getRaciMatrix();
		String fSave=mSWT.getFileSave();
		if(resultOrgUnitVsActorMatrix==null){
			//Validation error
			return null;
		}
		if(resultActorVsRoleMatrix==null){
			//Validation error
			return null;
		}
		if(resultRaciMatrix==null){
			//Validation error
			return null;
		}
		if(fSave!=null){
		...
		File fileOrgUnitVsActorMatrix;
		File fileActorVsRoleMatrix;
		File fileRaciMatrix;
		fileOrgUnitVsActorMatrix = new File(resultOrgUnitVsActorMatrix);
		fileActorVsRoleMatrix=new File(resultActorVsRoleMatrix);
		fileRaciMatrix=new File(resultRaciMatrix);
		if(!fileOrgUnitVsActorMatrix.exists() 
				|| !fileOrgUnitVsActorMatrix.exists() || !fileRaciMatrix.exists()){
			//Validation Error
			return null;
		}
		try
		{
			FileInputStream fileIn = new FileInputStream(fileOrgUnitVsActorMatrix);
			ObjectInputStream in = new ObjectInputStream(fileIn);
			orgUnitVsActorMatrix = (ResultSerializable) in.readObject();
			fileIn = new FileInputStream(fileActorVsRoleMatrix);
			in = new ObjectInputStream(fileIn);
			actorVsRoleMatrix = (ResultSerializable) in.readObject();
			fileIn = new FileInputStream(fileRaciMatrix);
			in = new ObjectInputStream(fileIn);
			raciMatrix = (ResultSerializable) in.readObject();
			in.close();
			fileIn.close();
		}catch(IOException i)
		{
		...
		List<MatrixResult> listOrgUnitVsActorMatrix=null;
		List<MatrixResult> listActorVsRoleMatrix=null;
		List<RaciMatrixResult> listRaciMatrix=null;
		if(orgUnitVsActorMatrix!=null && actorVsRoleMatrix!=null && raciMatrix!=null){
			listOrgUnitVsActorMatrix=(List<MatrixResult>)orgUnitVsActorMatrix.getResultEntries();
			listActorVsRoleMatrix=(List<MatrixResult>)actorVsRoleMatrix.getResultEntries();
			listRaciMatrix= (List<RaciMatrixResult>) raciMatrix.getResultEntries();
		}
		else{
			//Validation Error
			return null;
...
//		}
//		
//		listResults = new ArrayList<DecompositionFunctionalResult>();
...
//			String rol= DiagramModelUtil.getValue(relationship, "rol");
//			if(elements1.get(idTarget)!=null && rol!=null){
//				elements1.get(idTarget).getIdSource().put(idSource,rol);
//			}
//		}
		return null;
	}	
	...
	public Object executeFunction() throws Exception {
		...
		for(WElement element:elements){
			DecompositionFunctionalResult result=new DecompositionFunctionalResult(element, orderedElements2);
...
//		return ret;
	...
	public String getDescription() {
		return "This function returns a RACI matrix between two catalogs.\n"
				+ "Input parameters:\n"
				+ "1. File path of the first catalog\n"
				+ "2. File path of the second catalog\n"
				+ "3. File path where the RACI matrix will be saved\n"
				+ "Output:\n"
...
//		return ret;
	...
	public String[] inNames() {
		...
		return new String[]{"Catalog 1;","Catalog 2:", "File path: "};
	}
